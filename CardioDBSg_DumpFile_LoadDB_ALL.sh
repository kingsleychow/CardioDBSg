...
for RUN in ${RUNS}; do 
	...
	echo "### Loading data into Runs ###" >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log
        $MYSQL_PATH/bin/mysqlimport --local --lock-tables --verbose --ignore --fields-terminated-by=',' --lines-terminated-by='\n' --ignore-lines=1 -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB $CARDIODBS_PATH/Dump/Runs/Runs.${RUN}.csv >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log 2>&1
        echo " =====> Done!"

        printf "  Loading data into Samples"
	echo "### Loading data into Samples ###" >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log
        $MYSQL_PATH/bin/mysqlimport --local --lock-tables --verbose --ignore --fields-terminated-by=',' --lines-terminated-by='\n' --ignore-lines=1 -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB $CARDIODBS_PATH/Dump/Samples/Samples.${RUN}.csv >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log 2>&1
	echo " =====> Done!"
	echo "====================="
	...
	for CALLER in ${CALLERS} ;
	  ...
	  perl ${PARSER} --run_name ${RUN} --db $DB --dbhost $DB_HOST --dbport $DB_PORT --dbuser $DB_USER --dbpass $DB_PASS > ${CARDIODBS_PATH}/Dump/${CALLER}/${CALLER}.${RUN}.txt 2>> log_cardiodbs_$DATE/${CALLER}.$DATE.$RUN.log
	  echo " =====> Done!"
	  ...
	  printf "  Loading data into ${CALLER}"
		...
		echo "### Loading data into ${CALLER} ###" >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log
          	$MYSQL_PATH/bin/mysqlimport --local --lock-tables --verbose --replace -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/${CALLER}/${CALLER}.${RUN}.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log 2>&1
          	echo " =====> Done!";
	  fi
	done
	echo "====================="
	...
	perl ${CARDIODBS_PATH}/bin/make_Enrichments.pl --run_name $RUN --db $DB --dbhost $DB_HOST --dbport $DB_PORT --dbuser $DB_USER --dbpass $DB_PASS --CARDIODB_ROOT ${CARDIODBS_PATH} 2>> log_cardiodbs_$DATE/Enrichment.$DATE.$RUN.log
	echo " =====> Done!";
	...
	if [[ ${RUN} =~ "_M02463_" ]]; then
	  ...
	  echo "### Loading data into SampleEnrichments and CodingEnrichments ###" >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log
          $MYSQL_PATH/bin/mysqlimport --local --lock-tables --verbose --replace -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/Enrichments/SampleEnrichments.${RUN}.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log 2>&1
          $MYSQL_PATH/bin/mysqlimport --local --lock-tables --verbose --replace -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/Enrichments/CodingEnrichments.${RUN}.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.$RUN.log 2>&1
          echo " =====> Done!";
        fi
...
perl ${CARDIODBS_PATH}/bin/make_HasFound.pl --sql --db $DB --dbhost $DB_HOST --dbport $DB_PORT --dbuser $DB_USER --dbpass $DB_PASS --CARDIODB_ROOT ${CARDIODBS_PATH} --new_entries --check_alleles > ${CARDIODBS_PATH}/Dump/HasFound/HasFound.added.txt 2>> log_cardiodbs_$DATE/HasFound.added.$DATE.log
echo " =====> Done!"

### Database ###
printf "  Loading data into HasFound"
echo "### Loading data into HasFound ###" >> log_cardiodbs_$DATE/mysqlimport.$DATE.log
$MYSQL_PATH/bin/mysqlimport --local --lock-tables --replace -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/HasFound/HasFound.added.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.log 2>&1
echo " =====> Done!"
...
perl ${CARDIODBS_PATH}/bin/make_HasFound.pl --sql --db $DB --dbhost $DB_HOST --dbport $DB_PORT --dbuser $DB_USER --dbpass $DB_PASS --CARDIODB_ROOT ${CARDIODBS_PATH} --new_entries > ${CARDIODBS_PATH}/Dump/HasFound/HasFound.added.colocated.txt 2>> log_cardiodbs_$DATE/HasFound.added.$DATE.log
echo " =====> Done!"
...
$MYSQL_PATH/bin/mysqlimport --local --lock-tables --ignore -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/HasFound/HasFound.added.colocated.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.log 2>&1
echo " =====> Done!"
...
$MYSQL_PATH/bin/mysql -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB < ${CARDIODBS_PATH}/SQL/update_UnifiedCalls.has_found.sql >> log_cardiodbs_$DATE/mysql.$DATE.log 2>&1
echo " =====> Done!"
...
$MYSQL_PATH/bin/mysql -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB < ${CARDIODBS_PATH}/SQL/update_V2dbNSFP.sql >> log_cardiodbs_$DATE/mysql.$DATE.log 2>&1
echo " =====> Done!"
...
for RUN in ${RUNS} ; do
	...
	$MYSQL_PATH/bin/mysql -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB -e "call insert_metacalls_by_runname_new('${RUN}')" >> log_cardiodbs_$DATE/mysql.$DATE.$RUN.log 2>&1
	echo " =====> Done!"
done
...
perl ${CARDIODBS_PATH}/bin/make_V2Ensembls.pl --sql --all --new_entries --v2ensx --annotation --db $DB --dbhost $DB_HOST --dbport $DB_PORT --dbuser $DB_USER --dbpass $DB_PASS --CARDIODB_ROOT ${CARDIODBS_PATH} 2>> log_cardiodbs_$DATE/V2Ensembls.$DATE.log
echo " =====> Done!"
...
$MYSQL_PATH/bin/mysqlimport --local --lock-tables --replace -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/V2Ensembls/V2Ensembls.all.added.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.log 2>&1
echo " =====> Done!"
...
$MYSQL_PATH/bin/mysqlimport --local --lock-tables --replace -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/V2dbSNPs/V2dbSNPs.all.added.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.log 2>&1
echo " =====> Done!"
...
$MYSQL_PATH/bin/mysqlimport --local --lock-tables --replace -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/V2Phens/V2Phens.all.added.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.log 2>&1
echo " =====> Done!"
...
$MYSQL_PATH/bin/mysqlimport --local --lock-tables --replace -h $DB_HOST -P $DB_PORT -u$DB_USER -p$DB_PASS $DB ${CARDIODBS_PATH}/Dump/V2Freqs/V2Freqs.all.added.txt >> log_cardiodbs_$DATE/mysqlimport.$DATE.log 2>&1
echo " =====> Done!"
