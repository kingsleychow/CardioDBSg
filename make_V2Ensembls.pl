...
MAIN:{
  ...
  if(defined $is_sql){
      ...
      $aux = $not_in_v2ensembl ? $aux=~/where/?$aux.' and in_ensembl=0' : 'where in_ensembl=0':$aux;
      $aux = $new_entries ? $aux=~/where/?$aux.' and is_new=1' : 'where is_new=1':$aux;
...
sub v2ensx{
  ...
  #There is no TranscriptVariation
  ...
  #There is a TranscriptVariation
      ...
      foreach my $tr_v(@{$new_vf->get_all_TranscriptVariations}) {
          ...
          } #end of unless-defined 
          
          #There is a Transcript object
          else{
  ...
  } #end of unless(@{$new_vf->get_all_TranscriptVariations})-else
} #end of sub v2ensx
...
sub find_existing{
  ...
  if(defined($new_vf->adaptor->db)) {
      my $sth = $new_vf->adaptor->db->dbc->prepare(qq{
          ...
          FROM variation_feature WHERE seq_region_id = ? AND seq_region_start = ? AND seq_region_end = ?
  ...
  } #end of if(defined($new_vf->adaptor->db))
    
  return undef;
} #end of sub find_existing
...
sub is_var_novel {
  ...
  } #end of if(defined($check_alleles))
    
  return $is_novel;
} #end of sub is_var_novel
